...
def main():
    ...
    # Filter for target path/row AND low cloud cover (<30%)
    low_cloud_items = []
    for item in items:
        ...
        # Check if matches target path/row AND has low cloud cover
        if (str(item_path) == TARGET_PATH and 
    ...
    summary = {
        ...
        "weather_files_downloaded": len([v for v in weather_downloads.values() if v.get("file")]),
        "scenes": all_downloaded_scenes,
        "weather_data": weather_downloads
    }
    ...
    weather_files = len([v for v in weather_downloads.values() if v.get("file")])
    
    print(f"\nTotal Bands Downloaded: {total_bands}")
    print(f"Weather Files (7:00 UTC): {weather_files}")
