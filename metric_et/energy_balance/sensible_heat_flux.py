...
class SensibleHeatFlux:
    ...
    def calculate(
        ...
        if dt_a is not None:
            ...
            if dt_a == 0.0:
                logger.warning(f"WARNING: calibration_a is zero! This will cause H=0 everywhere.")
                logger.warning(f"This indicates calibration failure - check anchor pixel selection and dT_hot validation.")
        else:
            # Calculate dT
            ...
            # Energy balance constraint: H cannot exceed available energy
            # (Rn - G), but G is typically small, so we use Rn as approximation
            H = np.where(rn < 0, np.minimum(H, rn * 0.5), np.minimum(H, rn * 0.9))
